	...
	public void sessionOpened(final IoSession session) throws Exception {
		...
		new IoFutureListener<ConnectFuture>() {
			...
			public void operationComplete(ConnectFuture future) {
				try {
					IoSession futureSession = future.getSession();

					futureSession.setAttribute(OTHER_IO_SESSION, session);
					session.setAttribute(OTHER_IO_SESSION, future.getSession());

					futureSession.resumeRead();
					futureSession.resumeWrite();
				} catch (RuntimeIoException e) {
					session.close(true); // Connect failed
				} finally {
					session.resumeRead();
					session.resumeWrite();
				}
